...
def make_retro(*, game, state=None, max_episode_steps=20000, seed=10, **kwargs):
    if state is None:
        state = retro.State.DEFAULT
    env = retro.make(game, state, **kwargs)
    env = StochasticFrameSkip(env, n=4, stickprob=0.25, SEED=seed)
...
def main():
    ...
    def make_env(state_name, seed_=10):
        def _init():
            env = make_retro(game=args.game, state=state_name, scenario=args.scenario, seed=seed_)
    ...
    if int(args.curriculum_select) == 0: 
        ...
        env_seeds = [args.seed + i for i in range(16)]   
        make_envs = [make_env("Start", seed_=env_seeds[i]) for i in range(16)]
    ...
    #print(type(args.lr), args.lr, type(args.clip), args.clip)
    
    model = PPO(
        policy="CnnPolicyImproved",
        env=venv,
        learning_rate=lambda f: f * args.lr,
        n_steps=128,
        batch_size=256,
        n_epochs=4,
        gamma=0.99,
